#!/bin/bash -xe
export HOME="/root"
SSM_DB_PASSWORD="/finaltask/db-password"
...
EFS_ID=$(aws efs describe-file-systems --query 'FileSystems[?Name==`EFS`].FileSystemId' --region $REGION --output text)
